...
// Adapted for Epsilon GlslSandBox
...
// ---- Procedural hash / noise (replaces iChannel0 texture lookups) ----
...
float noise(in vec2 x){return hash(x*100.0);}
float fbm(in vec2 p)
{
    float z=.5;
    float rz = 0.;
...
void main()
{
    ...
    vec2 mo = vec2(-1.759, 0.483);      // locked view
